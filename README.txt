...
1.  **Inferencia Estadística Corregida:** Se demostró que la prueba T de Student inicial sobre 3,600 muestras presentaba un sesgo por autocorrelación. Se agrupó la latencia a nivel de cliente (n=4) y se aplicó la prueba U de Mann-Whitney, manteniendo la significancia estadística real.
...
-   `/scripts/`: Los scripts que permiten reproducir las pruebas.
    ...
    -   `analisis_p2.py`, `analisis_wan.py`, `analisis_grid.py`: Scripts estadísticos de Python.
...
-   `/documentos/`: Textos generados (como `REESCRITURA_P2.md`) y tablas estadísticas en `.txt`.
